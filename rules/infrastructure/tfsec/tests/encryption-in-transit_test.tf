# Test fixtures for KIRBY-SEC-007 / EEDOM-AWS-012 — Encryption in Transit
...
#
# TFSEC LIMITATION: EEDOM-AWS-012 flags any listener whose protocol attribute
...
resource "aws_lb_listener" "pass_https_with_ssl_policy" {
  ...
  port              = 443
  protocol          = "HTTPS" # PASS: encrypted protocol — satisfies EEDOM-AWS-012
...
resource "aws_lb_listener" "pass_tls_nlb" {
  ...
  port              = 443
  protocol          = "TLS" # PASS: TLS for NLB — satisfies EEDOM-AWS-012
...
resource "aws_alb_listener" "pass_alb_alias_https" {
  ...
  port              = 8443
  protocol          = "HTTPS" # PASS: aws_alb_listener alias — satisfies EEDOM-AWS-012
...
# FAIL: plain HTTP listener — transmits data unencrypted — fails EEDOM-AWS-012
resource "aws_lb_listener" "fail_http_plain" {
  load_balancer_arn = "arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/my-alb/1234567890abcdef"
  port              = 80
  protocol          = "HTTP" # FAIL: plaintext HTTP — fails EEDOM-AWS-012
...
# FAIL: HTTP redirect-only listener — architecturally acceptable but flagged by EEDOM-AWS-012
# due to tfsec's attribute-level evaluation (see TFSEC LIMITATION note at top of file).
# Add tfsec-ignore:EEDOM-AWS-012 with a comment if this pattern is intentional.
resource "aws_lb_listener" "fail_http_redirect_no_suppress" {
  load_balancer_arn = "arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/my-alb/1234567890abcdef"
  port              = 80
  protocol          = "HTTP" # FAIL: flagged by EEDOM-AWS-012 despite redirect intent — add tfsec-ignore to suppress
...
resource "aws_lb_listener" "pass_http_redirect_suppressed" {
  load_balancer_arn = "arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/my-alb/1234567890abcdef"
  port              = 80
  # tfsec-ignore:EEDOM-AWS-012 — HTTP-to-HTTPS permanent redirect only; no data in transit
  protocol = "HTTP"
...
resource "aws_alb_listener" "fail_alb_alias_http" {
  load_balancer_arn = "arn:aws:elasticloadbalancing:us-east-1:123456789012:loadbalancer/app/my-alb2/abcdef1234567890"
  port              = 80
  protocol          = "HTTP" # FAIL: aws_alb_listener alias plaintext HTTP — fails EEDOM-AWS-012
